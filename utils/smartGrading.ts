 ...
 * Supports multiple correct answer indices.
 */
export const isCorrectMC = (
  ...
  if (selectedIndex === correctAnswer) return true;
  if (alternativeCorrectAnswers?.includes(selectedIndex)) return true;
  return false;
};
...
export const getAllCorrectIndices = (
  ...
  if (alternativeCorrectAnswers) {
    indices.push(...alternativeCorrectAnswers);
  }
...
export const smartCheckMC = (
  ...
  const autoEquivalent = findEquivalentOptions(options, correctAnswer);
...
export const getAllSmartCorrectIndices = (
  ...
  const autoDetected = findEquivalentOptions(options, correctAnswer);
