...
export const WeeklyReportAggregator: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (existing) {
      ...
      // Create initial sample sessions for this week (e.g., 3 sessions: T2, T4, T6)
      const startDay = (selectedWeek - 1) * 7 + 1;
      const mStr = String(selectedMonth).padStart(2, '0');
      const defaultSessions: WeeklySessionConfig[] = [
        {
          id: `w_s_${selectedWeek}_1`,
          name: 'Buổi 1 (Thứ 2)',
          date: `${String(Math.min(startDay, 28)).padStart(2, '0')}/${mStr}/${selectedYear}`,
          columns: [
            { key: 'video', label: 'Video BTVN' },
            { key: 'btvn', label: 'BTVN' },
            { key: 'oldLesson', label: 'Kiểm tra bài cũ' }
          ]
        },
        {
          id: `w_s_${selectedWeek}_2`,
          name: 'Buổi 2 (Thứ 4)',
          date: `${String(Math.min(startDay + 2, 28)).padStart(2, '0')}/${mStr}/${selectedYear}`,
          columns: [
            { key: 'video', label: 'Video BTVN' },
            { key: 'btvn', label: 'BTVN' },
            { key: 'oldLesson', label: 'Kiểm tra bài cũ' }
        ...
        {
          id: `w_s_${selectedWeek}_3`,
          name: 'Buổi 3 (Thứ 6)',
          date: `${String(Math.min(startDay + 4, 28)).padStart(2, '0')}/${mStr}/${selectedYear}`,
          columns: [
            { key: 'video', label: 'Video BTVN' },
            { key: 'online', label: 'Bài tập online' },
            { key: 'btvn', label: 'BTVN' },
            { key: 'oldLesson', label: 'Kiểm tra bài cũ' }
          ]
        }
  ...
  const handleScoreChange = (
    ...
    if (trimmed.toLowerCase() === 'x') {
      ...
      if (!isNaN(parsed) && parsed >= 0 && parsed <= 10) {
        cleanVal = Math.round(parsed * 100) / 100;
      } else {
        return;
      }
    }
