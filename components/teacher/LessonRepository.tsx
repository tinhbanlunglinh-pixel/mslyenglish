    ...
    return (
      ...
      >
        <div className="space-y-3">
          {/* Top badges: Lớp + Trạng thái hạn nộp */}
          ...
          {/* Tiêu đề & Topic */}
          <div>
            <h4 className="font-black text-slate-900 text-base leading-snug line-clamp-2 group-hover:text-purple-700 transition-colors">
              {assign.title}
            </h4>
      ...
      <div className="bg-slate-50 rounded-3xl max-w-5xl w-full max-h-[95vh] flex flex-col shadow-2xl border-4 border-purple-300 overflow-hidden">
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-6">
          ...
          {lesson?.reading?.passage && (
            <div className="bg-white p-5 sm:p-6 rounded-3xl shadow-sm border border-slate-200 space-y-3">
              <h4 className="text-base font-black text-brand-900 uppercase flex items-center gap-2">
                <span>📖</span> PHẦN 3: BÀI ĐỌC HIỂU - {lesson.reading.title}
              </h4>
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-slate-800 leading-relaxed text-sm font-serif italic">
                "{lesson.reading.passage}"
              </div>
              {lesson.reading.translation && (
                <details className="bg-brand-50/50 p-3 rounded-xl border border-brand-100 text-xs text-slate-600">
