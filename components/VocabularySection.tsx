...
import { playGeminiTTS, stopTTS, generateAudioFromContent, createBritishSpeechUtterance } from '../services/geminiService';
...
export const VocabularySection: React.FC<VocabularySectionProps> = ({ items = [], topicTitle = '' }) => {
  ...
  // Speak a text with Web Speech API using unified British English female voice
  const speakWordPromise = (text: string, rate = 0.88): Promise<void> => {
    return new Promise((resolve) => {
      ...
      const utterance = createBritishSpeechUtterance(text, rate);
